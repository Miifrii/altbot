...
async def sync_existing_tickets(bot: commands.Bot):
    ...
    for guild in bot.guilds:
        print(f"[TICKETS] Проверка сервера: {guild.name} (ID: {guild.id})")
        
        # Получаем все категории тикетов из конфига
        categories_config = CONFIG.get("categories", {})
        print(f"[TICKETS] Конфиг категорий: {categories_config}")
        ...
        print(f"[TICKETS] ID категорий для проверки: {category_ids}")
        
        for category_id in category_ids:
            print(f"[TICKETS] Проверка category_id: {category_id}")
            if not category_id:
                print("[TICKETS] Пропущен - category_id is None")
                continue
            
            category = guild.get_channel(category_id)
            if not category:
                print(f"[TICKETS] Категория {category_id} не найдена на сервере")
                continue
            if not isinstance(category, discord.CategoryChannel):
                print(f"[TICKETS] {category_id} не является категорией")
                continue
            
            print(f"[TICKETS] Сканирование категории '{category.name}' (ID: {category_id}) на сервере {guild.name}")
